# -*- coding: utf-8 -*-
"""1.Online data obtain from API-recommend by movie category-liao.ipynb
...
df.to_csv('train_top10000_week10.csv', columns=['title', 'overview', 'genre_ids', 'id', 'popularity'])
...
df.to_csv('train_upcoming_week10.csv', columns=['title', 'overview', 'genre_ids', 'id', 'popularity'])

with open('train_upcoming_week10.csv', 'r') as f:
    print(len(f.readlines()))

data1=pd.read_csv("train_upcoming_week10.csv")

data2= pd.read_csv('train_top10000_week10.csv', index_col=None, header=0, engine='python')
...
any(result.duplicated('id')) #ID有重复

result.isnull().any() #缺失值查询

result.isnull().sum() #每一行都存在有缺失值的情况出现

result_2 = result.drop_duplicates(subset='id', keep='first', inplace=False) #删掉有重复的ID
...
no_missing_result.to_csv('no_missing_result_week10_3.csv')

"""得到目标数据集，接着再对数据集进行标签分离处理。
