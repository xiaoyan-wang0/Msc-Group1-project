...
f4=open('upcoming_movie_list.pckl','wb')
pickle.dump(upcoming1000_movies,f4)
f4.close()
print('Done!')

f4=open('upcoming_movie_list.pckl','rb')
upcoming1000_movies=pickle.load(f4)
f4.close()

len(upcoming1000_movies)

"""**Only 467 upcoming movies so far**"""

with open("upcoming_movie_list.pckl", "rb") as f:
    object = pkl.load(f)
    
df = pd.DataFrame(object)
df.to_csv(r'upcoming_movies.csv')

with open('upcoming_movies.csv', 'r') as f:
    print(len(f.readlines()))

df = pd.read_csv('upcoming_movies.csv', index_col=None, header=0, engine='python')
data = df.values

df.to_csv('train_upcoming.csv', columns=['title', 'overview', 'genre_ids', 'id'])

with open('train_upcoming.csv', 'r') as f:
    print(len(f.readlines()))